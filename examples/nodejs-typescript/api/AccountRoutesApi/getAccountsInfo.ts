import {
  AccountInfoDTO,
  AccountRoutesApi,
  AccountRoutesApiGetAccountsInfoRequest,
  Configuration,
...
(async () => {
  ...
  const requestParameters: AccountRoutesApiGetAccountsInfoRequest = {
    accountIds: {
      'addresses': ['NCSIOEWE2364XXP65426W3RUGBRYOAGR3KMMCIA']
    }
  };
  const response: AxiosResponse<AccountInfoDTO[], any> = await accountRoutesApi.getAccountsInfo(requestParameters);
